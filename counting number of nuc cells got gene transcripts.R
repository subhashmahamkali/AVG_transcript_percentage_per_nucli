...
for (folder_path in folders) {
  ...
  for (file_path in target_files) {
    ...
    filtered_df <- df[!is.na(df$Cell.ID) & df$Cell.ID > 1 & df$Total.transcript.. != 0 & df$Nucleus.Area..px. != 0, ] 
    
    # Update the "Total cells expressed" value in the first row only
    Total_cells =  df$`Nuc_cells_exp`[1] <- nrow(filtered_df)
    print(Total_cells)
    ...
    Nuc_cells_exp = rbind(Nuc_cells_exp, data.frame(file_name = file_name, Total_cells = Total_cells))
    # Write the results to a new Excel file
    write.xlsx(Nuc_cells_exp, "Nuc_cells.xlsx", row.names = FALSE)
  }
  }
